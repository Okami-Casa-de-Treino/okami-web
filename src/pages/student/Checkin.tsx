...
const Checkin: React.FC = () => {
  ...
  return (
    <div className="min-h-screen overflow-y-auto">
      <div className="space-y-6 p-4 sm:p-6 pb-8">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            >
              <QrCode size={20} />
              <span className="text-sm sm:text-base">Scanner QR</span>
            </button>
            <button 
              onClick={handleManualCheckin}
              disabled={isCreating}
              className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-sm disabled:opacity-50"
            >
              <UserCheck size={20} />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Aulas de Hoje</h2>
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {isLoading ? (
                ...
                todayClasses.map((classItem) => (
                  <div key={classItem.id} className="flex items-center gap-4 p-3 sm:p-4 rounded-lg border border-gray-100 hover:border-blue-200 hover:bg-blue-50/50 transition-all">
                    <div className="flex-shrink-0">
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center font-semibold text-xs sm:text-sm ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h4 className="font-semibold text-gray-900 text-sm sm:text-base truncate pr-2">{classItem.name}</h4>
                      ...
                      <p className="text-xs sm:text-sm text-gray-600 mb-2 truncate">{classItem.teacher} • {classItem.room}</p>
                      <div className="flex items-center justify-between">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Check-ins Recentes</h2>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                <span className="text-sm text-gray-500 hidden sm:inline">Tempo real</span>
              </div>
            </div>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {isLoading ? (
                ...
                recentCheckins.map((checkin) => (
                  <div key={checkin.id} className="flex items-center gap-3 sm:gap-4 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                    <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                      {checkin.avatar}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="font-medium text-gray-900 text-sm sm:text-base truncate pr-2">{checkin.studentName}</p>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <p className="text-xs sm:text-sm text-gray-600 truncate pr-2">{checkin.className}</p>
                        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              ...
              <div className="text-left">
                <p className="font-medium text-gray-900 text-sm sm:text-base">Scanner QR Code</p>
                <p className="text-xs sm:text-sm text-gray-500">Check-in automático</p>
              </div>
            </button>
            <button 
              onClick={handleManualCheckin}
            ...
            <button className="flex items-center gap-3 p-4 border border-gray-200 rounded-lg hover:border-purple-300 hover:bg-purple-50 transition-all">
              ...
              <div className="text-left">
                <p className="font-medium text-gray-900 text-sm sm:text-base">Lista de Presença</p>
                <p className="text-xs sm:text-sm text-gray-500">Visualizar relatório</p>
              </div>
            </button>
          </div>
        </div>
